...
import (
	...
	"github.com/sahajsoft/provider-unleash/apis"
	"github.com/sahajsoft/provider-unleash/apis/v1alpha1"
	"github.com/sahajsoft/provider-unleash/config"
	"github.com/sahajsoft/provider-unleash/internal/clients"
	"github.com/sahajsoft/provider-unleash/internal/controller"
	"github.com/sahajsoft/provider-unleash/internal/features"
)

func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for Unleash").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-unleash"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-unleash",
		Cache: cache.Options{
	...
	kingpin.FatalIfError(apis.AddToScheme(mgr.GetScheme()), "Cannot add Unleash APIs to scheme")
	...
	kingpin.FatalIfError(controller.Setup(mgr, o), "Cannot setup Unleash controllers")
